...
class Paginator:
    ...
    def __init__(self, db: SQLAlchemy, req: PaginatorReq = None):
        if req is not None:
            self.current_page = req.current_page.data or 1
            self.page_size = req.page_size.data or 20
        self.db = db
    ...
    def paginate(self, select) -> list[Any]:
        ...
        # 2.计算总页数+总条数
        self.total_record = p.total or 0
        self.total_page = math.ceil((p.total or 0) / self.page_size)
